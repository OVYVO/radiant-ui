<template>
  <el-button v-bind="$attrs" v-if="!props.exportAll" type="primary" @click="emits('click', '')">
    <template #default>
      <img :src="props.actType == 'input' ? Input : Export" alt="" style="width: 1em; height: 1em" />
      &nbsp;{{ props.actType == "input" ? `${props.prefix}导入${props.suffix}` : `${props.prefix}导出${props.suffix}` }}
    </template>
  </el-button>
  <el-dropdown v-bind="$attrs" v-else split-button type="primary" @click="emits('click', 'all')">
